...
keyboard();
            ...
            time_resolution = 200;
            [basis whole_grain_indices] = build_basis(residual, partials_list, time_resolution);
            
            ls_basis = basis + 1e9;
            ls_basis = ls_basis - 1e9;

            % Do least squares
            [coeffs, A, mu, C, c, gamma] = LARS(ls_basis, residual);

            % Build approximation
            appx = basis(:,whole_grain_indices) * coeffs(end,whole_grain_indices)';            
...
function [basis, whole_grain_indices] = build_basis(section, partials_list, time_resolution)
    ...
    grain_wavenumbers = floor(partial_cycs_per_samp * time_resolution) * 2;
    grain_wavenumbers(find(grain_wavenumbers < 4)) = 4;
    ...
    for k = 1:size(partials_list,1)
        ...
        for j = 1:grain_counts(k)
            ...
            recip_vector_mag = 1.0 / sqrt(sum(basis(:,column_number) .* basis(:,column_number)));
            basis(:,column_number) = basis(:,column_number) * recip_vector_mag;
