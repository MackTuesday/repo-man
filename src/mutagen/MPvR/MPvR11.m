...
function [grains, residual] = ...
   extract_frequency(section, rads_per_sample, threshold_significance)
    ...
    kernel_start = 1 - ceil(analysis_wavenum * pi / rads_per_sample);
    kernel_end = -kernel_start;
    filter_kernel_sinus = cos([kernel_start:kernel_end]' * rads_per_sample);
    kernel_window_function = ...
        0.5 * (1 + cos([kernel_start:kernel_end]' * rads_per_sample / analysis_wavenum));
    filter_kernel = filter_kernel_sinus .* kernel_window_function;
    filter_kernel_amplitude = 1 / sqrt(sum(filter_kernel .* filter_kernel));
    % Yes, we mean to multiply by kernel_window_function again.
    filter_kernel = filter_kernel .* kernel_window_function * filter_kernel_amplitude;
    discrete_kernel_length = size(filter_kernel, 1); % Must be odd
    ...
    peak_locations = find(zero_crossings) + salient_filt_oput_begin;
    ...
    for peak_number = 1:size(array_of_peaks,1)
        ...
        % if (prob_stronger_response < threshold_significance)
            ...
            grain_amplitude = filter_kernel_amplitude * peak_value * 0.5 * 4 / ...
                              extraction_wavenum;
                    
            % Build grain
            frac_x_at_max_response = array_of_peaks(peak_number, 2);
            grain_start = ...
                floor(extraction_wavenum * -pi/rads_per_sample + frac_x_at_max_response) + 1;
            grain_end = ...
                ceil(extraction_wavenum * pi/rads_per_sample + frac_x_at_max_response) - 1;
            grain_domain = [grain_start:grain_end]' - frac_x_at_max_response;
            grain_window_function = ...
                0.5 * (1 + cos(grain_domain * rads_per_sample / extraction_wavenum));
            grain = grain_amplitude * cos(grain_domain * rads_per_sample) .* ...
                    grain_window_function;
keyboard();                              
            % Subtract grain from residual
            removal_start = this_peak_location - (discrete_kernel_length-1)/2 + grain_start;
            removal_end = this_peak_location - (discrete_kernel_length-1)/2 + grain_end;
...
figure(3); plot(fftconv(section, filter_kernel));

            grain_length = size(grain,1);
