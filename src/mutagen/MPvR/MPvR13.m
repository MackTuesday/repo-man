...
function [grains, residual] = ...
    ...
    salient_filt_oput_begin = analysis_kernel_discrete_length - 1 + ...
                              (ref_decomp_kernel_discrete_length-1)/2 - 1;
    last_peak_index = salient_filt_oput_begin;
    salient_filt_oput_end = size(section,1) - analysis_kernel_discrete_length + 1 - ...
                            (ref_decomp_kernel_discrete_length-1)/2 + 1;
    ...
    while 0 == 0
        ...
        ad_diffs = ad_kernel(2:end) - ad_kernel(1:end-1);
        signs_of_ad_diffs = sign(sign(ad_diffs) - 0.5);
        ad_zero_crossings = signs_of_ad_diffs(2:end) - signs_of_ad_diffs(1:end-1);
        ad_peak_int_locations = find(ad_zero_crossings) + 1;
        ...
        c1 = 0.5 * (to_the_right - to_the_left);
        c2 = 0.5 * (to_the_right + to_the_left) - c0;
        ad_peak_frac_locations = -c1 ./ (2 * c2);
        ...
        [abs_min_ad_scale, min_ad_scale_index] = min(abs(filt_values_at_ad_peaks ./ ad_peaks));
        min_ad_scale = filt_values_at_ad_peaks(min_ad_scale_index) / ...
                       ad_peaks(min_ad_scale_index);
        ...
        ad_removal_start = ceil(last_peak_index - (ref_decomp_kernel_discrete_length-1)/2);
        ...
        if (interpolating_frac_index > 0)
            decomp_kernel_start_time = -(ref_decomp_kernel_discrete_length-1)/2 - ...
                                       interpolating_frac_index;
        else
            decomp_kernel_start_time = -(ref_decomp_kernel_discrete_length-1)/2 - ...
                                       interpolating_frac_index + 1;
        endif
        decomp_kernel_end_time = decomp_kernel_start_time + ref_decomp_kernel_discrete_length;
        if (interpolating_frac_index == 0)
            decomp_kernel_end_time = decomp_kernel_end_time - 1;
        endif
        decomp_kernel_sinus = cos([decomp_kernel_start_time:decomp_kernel_end_time]' * ...
                                  rads_per_sample);
        decomp_kernel_window = ...
           0.5 * (1 + cos([decomp_kernel_start_time:decomp_kernel_end_time]' * ...
                          rads_per_sample / decomp_wavenum));
        decomp_kernel = decomp_kernel_sinus .* decomp_kernel_window;
        decomp_kernel_amplitude = 1 / sqrt(sum(decomp_kernel .* decomp_kernel));
        ...
        removal_end = removal_start + decomp_kernel_discrete_length - 1;
